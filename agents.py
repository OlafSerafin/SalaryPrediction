...
#%%
# === MODIFIED: Now extracts and returns sources ===
def get_company_info(company_name: str, missing_info_keys: list[str]) -> dict:
    """
    Robust version: Searches first, extracts sources, then formats to JSON.
    Returns dict with company details AND 'sources' key containing list of URLs.
    """
    ...
    found_text = ""
    top_sources = []  # === ADDED: List to store source URLs ===
    ...
    try:
        ...
        if meta and meta.grounding_chunks and meta.grounding_supports:
            ...
            # 2. Iterate through supports to find used links
            used_links = []
    ...
    try:
        ...
        # === ADDED: Attach sources to result ===
...
def process_job_offer(job_description_text: str) -> dict:
    ...
    Returns dict with:
    - job_details: extracted and enriched job information
    ...
    if not company_name or company_name == 'Unknown':
        ...
        if missing_keys:
            ...
            if 'sources' in external_info:
                ...
                del external_info['sources']  # Remove before merging
    ...
    print(f"   -> Źródła użyte: {len(all_sources)}")
...
# === MODIFIED: Updated instructions to handle sources ===
job_offer_agent = LlmAgent(
    ...
    3. Present this data to the user in a professional format:
       ...
       - **IMPORTANT**: If sources are available, include a "Sources" section at the end.
       - If a field is 'Unknown', output "Not specified".
       
    4. **CRITICAL - Sources formatting**:
       Each source MUST be formatted EXACTLY like this:
       [🔗](actual_url)
       ...
       **Sources Used:**
       ...
       Do NOT add any text labels like "Source 1" or numbers. ONLY use the link emoji 🔗 as the clickable text.
    """
)

# === MODIFIED: Updated coordinator instructions ===
